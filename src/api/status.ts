...
/**
 * @description
 * en: 英语  zh: 中文 it: 意大利语 np：尼迫尔语
 * hi: 印度语 ko：韩语 es: 西班牙语 pt: 葡萄牙语
 */
export const HTTP_TEXT = {
  ...
  hi: {
    ...
    SERVICE_UNAVAILABLE: 'सेवा उपलब्ध नहीं है। यह निलंबन या रखरखाव के लिए समय है!',
...
export const getHttpStatusText = function (code: number | null, err?: any): string {
  ...
  for (const key in HTTP_STATUS) {
    if (HTTP_STATUS[key] === code) {
      const text = HTTP_TEXT[lang][key];
      return text;
    }
  }

  if (typeof err === 'object' && err.message && err?.message.indexOf('timeout') > -1) {
    return HTTP_TEXT[lang].TIMEOUT;
  }
  if (typeof err === 'object' && err.message && err?.message.indexOf('Network') > -1) {
